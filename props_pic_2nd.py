...
def props_pic(proposals, all_tag_annos, all_imgs):
    ...
    for i, img in enumerate(all_imgs):
        ...
        for prop in proposals[i]:
            ...
            if overlap(all_tag_annos[i][:4][np.newaxis, :], prop) != np.array([[0]]): # np.min(prop[1], all_tag_annos[1])
    ...
    for i, img_props in enumerate(rs_boxes):
        ...
        if len(rs_boxes[i]) != 0:
            for props in img_props:
                ...
                num_labels[:, 0] = num_labels[:, 0] * (rs_wh[i][1]/(props[2]-props[0]))
                num_labels[:, 2] = num_labels[:, 2] * (rs_wh[i][1]/(props[2]-props[0]))
                num_labels[:, 1] = num_labels[:, 1] * (rs_wh[i][0]/(props[3]-props[1]))
                num_labels[:, 3] = num_labels[:, 3] * (rs_wh[i][0]/(props[3]-props[1]))
