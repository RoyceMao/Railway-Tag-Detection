...
def draw_boxes_and_label_on_image(img, class_boxes_map):
    """
    this method using cv2 to show boxes on image with various class labels
    :param img:
    :param class_label_map: {1: 'Car', 2: 'Pedestrian'}
    :param class_boxes_map: {1: [box1, box2..], 2: [..]}, in every box is [bb_left, bb_top, bb_right, bb_bottom, prob]
    :return:
    """

    for c, boxes in class_boxes_map.items():
        for box in boxes:
            assert len(box) == 4, 'class_boxes_map every item must be [bb_left, bb_top, bb_width, bb_height, prob]'
            ...
            '''
    for b, gts in gt_boxes_map.items():
        for gt in gts:
            assert len(gt) == 5, 'class_boxes_map every item must be [bb_left, bb_top, bb_width, bb_height, prob]'
            gt_left = int(gt[0])
            gt_top = int(gt[1])
            gt_right = int(gt[2])
            gt_bottom = int(gt[3])
            ...
            prob = round(gt[4], 2)
            unique_color = _create_unique_color_uchar(b)
            cv2.rectangle(img, (gt_left, gt_top), (gt_right, gt_bottom), unique_color, 1)
            '''
            '''
...
if __name__ == "__main__":
    ...
    '''
    cls_map = ['bicycle','person', 'person']
    regr_map = [[16, 122, 81, 173], [23, 96, 67, 174], [132, 82, 202, 173]]
    probs = [0.9, 0.9, 0.9]
    '''
